  ...
  withComputed(({ subscriptionsWithDetails }) => ({
    ...
    getProgressClasses: computed(() => {
      return (progress: number): string => {
        let baseClasses =
          'transition-all duration-300 ease-in-out shadow-md bg-gradient-to-r striped-progress';
        ...
        if (progress > 50)
          return `${baseClasses} from-yellow-400 to-yellow-500`;
        if (progress > 40)
          return `${baseClasses} from-yellow-400 to-yellow-500`;
        if (progress > 30)
          return `${baseClasses} from-orange-400 to-orange-500`;
        if (progress > 20)
          return `${baseClasses} from-orange-400 to-orange-500`;
    ...
    subscriptionStats: computed(() => {
      ...
      return {
        total: subs.length,
        active: subs.filter(
          (sub: SubscriptionWithDetails) => !sub.status.expired
        ).length,
        expired: subs.filter(
          (sub: SubscriptionWithDetails) => sub.status.expired
        ).length,
        ...
        datasets: [
          {
            ...
            data: last6Months.map((month: string) => {
              return subs.filter((sub: SubscriptionWithDetails) => {
                const startDate = sub.details.subscriptionStartDate
                  ? new Date(sub.details.subscriptionStartDate)
                  : null;
                return (
                  startDate &&
          ...
          },
          {
            ...
            data: last6Months.map((month: string) => {
              return subs.filter((sub: SubscriptionWithDetails) => {
                const endDate = sub.details.subscriptionEndDate
                  ? new Date(sub.details.subscriptionEndDate)
                  : null;
                return (
                  endDate &&
    ...
    channelCount: computed(() => {
      return (type: SubscriptionType, customChannelCount?: number): number => {
        const calculator = new SubscriptionCalculator(
          type,
          1,
          TimeUnit.MONTHS,
