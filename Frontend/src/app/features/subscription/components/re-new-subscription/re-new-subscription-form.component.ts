    ...
    <form [formGroup]="renewForm" (ngSubmit)="onSubmit()" class="space-y-4 p-2">
      <div class="grid grid-cols-2 gap-4 gap-y-6">
        <div class="space-y-2">
          ...
          @if (renewForm.get('subscriptionType')?.invalid &&
          renewForm.get('subscriptionType')?.touched) {
          <div class="error">
            <small *ngFor="let error of getErrors('subscriptionType')">{{
              error
            }}</small>
          </div>
          }
        ...
        <div class="grid grid-cols-2 gap-3">
          ...
          @if ((renewForm.get('duration')?.invalid &&
          renewForm.get('duration')?.touched) ||
          (renewForm.get('timeUnit')?.invalid &&
          renewForm.get('timeUnit')?.touched)) {
          <div class="error col-span-2">
            <small *ngFor="let error of getErrors('duration')">{{
              error
            }}</small>
            <small *ngFor="let error of getErrors('timeUnit')">{{
              error
            }}</small>
          </div>
          }
        ...
        <div class="col-span-2">
          ...
          <div class="text-sm mt-3 px-3 text-gradient ">
            ...
            {{
              getTimeUnitLabel(
                renewForm.get('timeUnit')?.value || TimeUnit.MONTHS
              )
            }}
          </div>
        </div>
      ...
      <div class="flex justify-end space-x-3 mt-6 border-t border-gray-700">
        <button type="button" (click)="OnClose()" class="cancel-btn px-4 py-2">
          Cancel
        </button>
        <button
          type="submit"
          ...
          [ngClass]="{
            'btn-desactived-bg': !renewForm.valid,
            'btn-gradient-bg': renewForm.valid
          }"
        >
...
export class RenewSubscriptionFormComponent {
  ...
  private updateDisplayedValues() {
    const type = this.renewForm.get('subscriptionType')
