...
export class SubscriptionCalculator {
  ...
  constructor(
    ...
    if (!config) {
      ...
      this.channelCalc = new ChannelCalculator(
        SubscriptionType.BASIC,
        customChannelCount
      );
      return;
  ...
  calculateTimeBasedPrice(): number {
    switch (this.unit) {
      case TimeUnit.DAYS:
        return this.calculator.divide(30).multiply(this.duration).getValue();

      case TimeUnit.WEEKS:
        return this.calculator.divide(4).multiply(this.duration).getValue();

      case TimeUnit.MONTHS:
        return this.calculator.multiply(this.duration).getValue();

      case TimeUnit.YEARS:
        return this.calculator.multiply(this.duration * 12).getValue();
